...
async function getOrdersByUser(nguoiDung, role, page, limit) {
    ...
    const result = await request.query(`
        SELECT DH.MADH, DH.NGUOIMUA, DH.NGUOIBAN, DH.LOAIGIAODICH, DH.TRANGTHAI, DH.DIADIEMHEN, DH.THOIGIANHEN,
                ...
                CT.MAGT, CT.SOLUONG, CT.DONGIA, GT.TENGT, GT.LOAI, DX.MAGTDUOCDOI, DX.MAGTMANGDOI,
                DX.SOLUONGMANGDOI, GT_MANGDOI.TENGT AS TENGTMANGDOI, GT_MANGDOI.SOLUONG AS SOLUONGCONLAIMANGDOI,
                GT_MANGDOI.SOLUONGDANGGIU AS SOLUONGDANGGIUMANGDOI, GT_MANGDOI.TRANGTHAI AS TRANGTHAIMANGDOI,
                TK_MUA.TENTK AS TENNGUOIMUA, TK_BAN.TENTK AS TENNGUOIBAN, COUNT(*) OVER() AS TONGSO
        FROM DONHANG DH
        JOIN CHITIETDONHANG CT ON CT.MADH = DH.MADH
        JOIN GIAOTRINH GT ON GT.MAGT = CT.MAGT
        ...
        ORDER BY DH.NGAYCAPNHAT DESC, DH.MADH DESC
        OFFSET @OFFSET ROWS
...
async function getOrderDetail(maDH, nguoiDung) {
    ...
    WHEN EXISTS (
        SELECT 1
        FROM DANHGIA DG
        WHERE DG.MADH = DH.MADH
          AND DG.NGUOIDANHGIA = DH.NGUOIMUA
    )
    THEN 1
    ELSE 0
END AS DADANHGIA
